...
import { MessageSquare, AlertTriangle, Clock, Send, Check } from 'lucide-react';
  ...
  const handleReply = async (askId: string) => {
    ...
    try {
      const res = await fetch('/api/doctor/quick-ask', {
        ...
        body: JSON.stringify({ askId, replyText })
      });
      ...
      setAsks((prev) =>
        prev.map((q) =>
          q.id === askId
            ? { ...q, status: 'answered', answer: replyText, answered_at: new Date().toISOString() }
            : q
        )
      );
      setReplyingTo(null);
  ...
  return (
    <div className="min-h-screen bg-[#F7F9FC]">
      <header className="sticky top-0 z-30 bg-white border-b border-slate-100 px-5 py-4 shadow-sm">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-xs text-slate-500">{pending.length} pending · {asks.filter((q) => q.is_urgent).length} urgent</p>
          </div>
      ...
      <div className="p-4 space-y-3">
        ...
        {displayed.map((ask: any, i) => {
          const patientName = ask.patient?.users?.display_name || 'Patient';
          const patientStatus = ask.patient?.status || 'stable';
          ...
          return (
            <motion.div
              ...
              className={`card overflow-hidden ${ask.is_urgent ? 'border-[#FF4757]/40' : ''}`}
            >
              {ask.is_urgent && (
              ...
              <div className="p-4">
                ...
                <div className="flex items-center gap-3 mb-3">
                  ...
                  <div className="flex-1">
                    ...
                    <div className="flex items-center gap-2 text-[10px] text-slate-400">
                      ...
                      {new Date(ask.created_at).toLocaleDateString('en-IN', { day: 'numeric', month: 'short' })}
                      {' · '}
                      {new Date(ask.created_at).toLocaleTimeString('en-IN', { hour: '2-digit', minute: '2-digit' })}
                      <span className="px-1.5 py-0.5 rounded-full bg-slate-100 capitalize">{patientStatus}</span>
                    </div>
                  </div>
                  {!isPending && (
                ...
                <div className={`p-3 rounded-xl text-sm leading-relaxed mb-3 ${
                  ask.is_urgent ? 'bg-red-50 border border-red-100 text-red-800' : 'bg-slate-50 text-slate-700'
                }`}>
                ...
                {ask.answer ? (
                  <div className="p-3 rounded-xl bg-blue-50 border border-blue-100">
                    ...
                    <p className="text-xs text-slate-700">{ask.answer}</p>
                  </div>
                ) : replyingTo === ask.id ? (
                  <motion.div
