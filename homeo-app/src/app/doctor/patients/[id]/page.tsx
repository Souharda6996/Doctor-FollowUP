...
import {
  ...
  MessageSquare, Edit, AlertTriangle, ChevronDown, ChevronUp,
  Clock, TrendingUp, FileText
} from 'lucide-react';
import { MOCK_PATIENTS, MOCK_CASE_HISTORY, MOCK_REMEDIES, MOCK_SYMPTOM_LOGS, MOCK_TIMELINE } from '@/lib/mockData';
import { getStatusColor, getStatusDot, formatDate, formatRelativeTime } from '@/lib/utils';
...
export default function PatientCaseFile() {
  ...
  const caseHistory = MOCK_CASE_HISTORY[patientId] ?? [];
  const remedies = MOCK_REMEDIES[patientId] ?? [];
  const logs = MOCK_SYMPTOM_LOGS[patientId] ?? [];
      ...
      <div className="gradient-header px-5 pt-5 pb-16 relative">
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-blue-100 text-sm mt-0.5">{patient.age}y · {patient.gender} · {patient.caseType}</p>
        ...
        {activeTab === 'overview' && (
          <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className="space-y-4">
            ...
            <div className="card p-4">
              ...
              <p className="text-slate-800 text-sm font-medium">{patient.chiefComplaint}</p>
            </div>
            ...
            <div className="card p-4 space-y-3">
              ...
              <div className="space-y-2 text-sm">
                ...
                <div className="flex items-center gap-2 text-slate-600">
                  ...
                  First visit: {formatDate(patient.createdAt)}
                </div>
                <div className="flex items-center gap-2 text-slate-600">
                  <Clock className="w-3.5 h-3.5 text-slate-400 flex-shrink-0" />
                  Last visit: {formatRelativeTime(patient.lastVisit)}
                </div>
                {patient.nextFollowUp && (
                  <div className="flex items-center gap-2 text-blue-600 font-medium">
                    <Calendar className="w-3.5 h-3.5 flex-shrink-0" />
                    Next follow-up: {formatDate(patient.nextFollowUp)}
                  </div>
                )}
              </div>
            </div>
            ...
            <div className="card border-purple-200 overflow-hidden">
              ...
              {showAIInsights && caseHistory[0]?.aiSummary && (
                <div className="px-4 pb-4 border-t border-purple-100">
                  <div className="bg-purple-50 rounded-xl p-3 mt-3">
                    <p className="text-sm text-purple-900 leading-relaxed">{caseHistory[0].aiSummary}</p>
                  </div>
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-4">
            ...
            {caseHistory.map((ch) => (
              <div key={ch.id} className="card p-4 space-y-4">
                ...
                <Section title="Physical Symptoms" items={ch.physicalSymptoms} />
                <InfoRow label="Mental State" value={ch.mentalState} />
                <InfoRow label="Emotional State" value={ch.emotionalState} />
                <InfoRow label="Sleep" value={`${ch.sleepPattern} (${ch.sleepQuality})`} />
                <Section title="Food Preferences" items={ch.foodPreferences} color="green" />
                <Section title="Food Aversions" items={ch.foodAversions} color="red" />
                ...
                {ch.doctorNotes && (
                  <div className="bg-blue-50 border border-blue-200 rounded-xl p-3">
                    ...
                    <p className="text-sm text-blue-900">{ch.doctorNotes}</p>
                  </div>
                )}
              </div>
            ))}
            ...
            {remedies.map((remedy) => (
              <div key={remedy.id} className="card p-4 space-y-3">
                <div className="flex items-start justify-between">
                  <div>
                    <h3 className="font-bold text-slate-900">{remedy.name}</h3>
                    <p className="text-sm text-purple-600 font-semibold">{remedy.potency}</p>
                  </div>
                  {remedy.response && (
                    <span className={`text-[10px] font-bold uppercase px-2 py-0.5 rounded-full border ${
                      remedy.response === 'excellent' ? 'bg-green-50 text-green-700 border-green-200' :
                      remedy.response === 'good' ? 'bg-blue-50 text-blue-700 border-blue-200' :
                      remedy.response === 'moderate' ? 'bg-yellow-50 text-yellow-700 border-yellow-200' :
                      'bg-red-50 text-red-700 border-red-200'
                    }`}>{remedy.response}</span>
                  )}
                </div>
                <div className="grid grid-cols-2 gap-2 text-xs">
                  <LabelValue label="Dosage" value={remedy.dosage} />
                  <LabelValue label="Frequency" value={remedy.frequency} />
                  <LabelValue label="Duration" value={remedy.duration} />
                  <LabelValue label="Start Date" value={formatDate(remedy.startDate)} />
                </div>
                <div className="bg-slate-50 rounded-xl p-3">
                  <p className="text-xs text-slate-500 font-semibold mb-1">Reason</p>
                  <p className="text-xs text-slate-700">{remedy.reason}</p>
                </div>
                {remedy.responseNotes && (
                  <div className="bg-green-50 border border-green-200 rounded-xl p-3">
                    <p className="text-xs text-slate-500 font-semibold mb-1">Response Notes</p>
                    <p className="text-xs text-green-800">{remedy.responseNotes}</p>
                  </div>
                )}
              </div>
            ))}
            ...
            {logs.map((log) => (
              <div key={log.id} className="card p-4 space-y-2">
                <div className="flex items-center justify-between">
                  <span className="text-xs font-semibold text-slate-500">{formatDate(log.date)}</span>
                  <div className="flex items-center gap-2">
                    <div className="flex items-center gap-1">
                      ...
                      <div className="flex gap-0.5">
                        {Array.from({ length: 10 }).map((_, i) => (
                          <div
                            key={i}
                            className={`w-2 h-2 rounded-full ${i < log.severity ? (log.severity > 6 ? 'bg-red-500' : log.severity > 3 ? 'bg-yellow-500' : 'bg-green-500') : 'bg-slate-200'}`}
                          />
                        ))}
                      </div>
                    </div>
                ...
                <div className="flex items-center gap-3 text-xs text-slate-500">
                  <span>Mood: <span className="font-semibold text-slate-700 capitalize">{log.mood}</span></span>
                  <span>Energy: <span className="font-semibold text-slate-700">{log.energy}/5</span></span>
                </div>
